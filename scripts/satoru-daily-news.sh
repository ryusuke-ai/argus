#!/bin/bash
# さとるデイリーニュース自動生成スクリプト
# crontab: 0 0 * * * /path/to/argus/scripts/satoru-daily-news.sh
#
...
echo "=== さとるデイリーニュース開始: $(date) ===" >> "$LOG_FILE"

# Claude CLIのログイン状態を確認
if ! claude --print -p "ok" > /dev/null 2>&1; then
  echo "ERROR: Claude CLIが未ログインです。" >> "$LOG_FILE"
  notify_slack ":rotating_light: Claude CLIのログインが切れています。\`claude\` を起動して \`/login\` を実行してください。（さとるデイリーニュースをスキップします）"
  exit 1
fi
...
if ! curl -s http://localhost:50032/v1/style_id_to_speaker_meta_all > /dev/null 2>&1; then
  ...
  notify_slack ":warning: COEIROINKが起動していません。さとるデイリーニュースをスキップします。"
  exit 1
fi
...
claude --print -p "今日のさとるデイリーニュースを作成してください。
...
動画のレンダリングは3分割してそれぞれレンダリングしてからffmpegで結合してください。" \
  >> "$LOG_FILE" 2>&1 || {
    ...
    notify_slack ":warning: さとるデイリーニュースの自動生成でエラーが発生しました。ログ: ${LOG_FILE}"
    exit 1
  }

echo "=== さとるデイリーニュース完了: $(date) ===" >> "$LOG_FILE"

# 古いログを30日で削除
find "$LOG_DIR" -name "satoru-daily-news-*.log" -mtime +30 -delete 2>/dev/null
