---
name: sns-podcast-creator
description: ポッドキャスト構成を生成。sns-publisher から自動委譲。
---

# SNS Podcast Creator

ポッドキャスト向けのエピソード構成を、リサーチから最適化まで4フェーズで生成する。
...
[Phase 2] 構成設計 → work/structure.json
    ↓
[Phase 3] コンテンツ生成 → work/podcast-episode.json
    ↓
[Phase 4] 最適化 → 最終コンテンツ
```
...
| Phase 3 | @phases/phase3-content.md   | 直接実行（Claude） | podcast-episode.json |
...
| ファイル                              | 用途           |
| ------------------------------------- | -------------- |
| @prompts/podcast-content-generator.md | コンテンツ生成 |
...
| @schemas/podcast-episode.schema.json | Phase 3 |
