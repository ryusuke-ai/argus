---
name: sns-threads-poster
description: Threads 向け投稿コンテンツを生成。sns-publisher から自動委譲。
---

# SNS Threads Poster

Threads 向けの投稿コンテンツを、リサーチから最適化まで4フェーズで生成する。
...
[Phase 2] 構成設計 → work/structure.json
    ↓
[Phase 3] コンテンツ生成 → work/threads-post.json
    ↓
[Phase 4] 最適化 → 最終コンテンツ
```
...
| Phase 3 | @phases/phase3-content.md   | 直接実行（Claude） | threads-post.json |
...
| ファイル                              | 用途           |
| ------------------------------------- | -------------- |
| @prompts/threads-content-generator.md | コンテンツ生成 |
...
| @schemas/threads-post.schema.json | Phase 3 |
