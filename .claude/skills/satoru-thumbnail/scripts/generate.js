#!/usr/bin/env node
/**
 * satoru-thumbnail - Satoru Thumbnail Generator
 *
 * さとるちゃんサムネイル生成スクリプト。
 ...
 */
async function generateImage(prompt, aspectRatio, model, imagePath = null, iconImagePath = null) {
  ...
  const textInstruction = "重要: 日本語のタイトルテキストを画像内に大きく目立つように配置してください。太字で読みやすいフォントを使用し、サムネイルとして機能するようにしてください。";
  ...
  if (iconImagePath) {
    ...
    instructionPrefix += "【アイコン/ロゴ参照画像】上記の画像を2行目のアイコン要素として使用してください。このロゴ/アイコンのデザインを忠実に再現し、サムネイル内の指定位置に配置してください。\n\n";
  }
  ...
  if (imagePath) {
    ...
    instructionPrefix += "【キャラクター参照画像】上記の画像のキャラクターを参照して、キャラクターの特徴（服装、髪型、顔の特徴など）を維持してください。\n\n";
  }
...
async function main() {
  ...
  if (values.mode === "right-layout") {
    if (!values.line1 || !values.line2 || !values.line3 || !values.line4) {
      console.error("Error: right-layout mode requires --line1, --line2, --line3, --line4");
      process.exit(1);
    }
    ...
    if (!values["top-text"] || !values["bottom-text"] || !values.objects) {
      console.error("Error: sandwich mode requires --top-text, --bottom-text, --objects");
      process.exit(1);
    }
