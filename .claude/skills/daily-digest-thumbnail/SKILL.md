---
name: daily-digest-thumbnail
description: ダイジェスト動画用サムネイル生成スキル
---

# daily-digest-thumbnail

ダイジェスト動画用のYouTubeサムネイルを生成。固定化されたレイアウトパターンで、簡単にサムネイルを作成できる。

## 使用タイミング

- 「サムネイル生成」「ダイジェストのサムネイル」と依頼されたとき
- ダイジェスト動画用のサムネイルが必要なとき
...
```bash
node .claude/skills/daily-digest-thumbnail/scripts/generate.js \
  --output <出力先パス> \
  --mode <モード> \
  [オプション]
```
...
> **注記**: このキャラクターは @../\_shared/characters/satoru.md（さとるちゃん）の視覚表現。性格・口調は共通キャラクター設定を参照。画像生成用のビジュアルプロンプトは `config.json` の `character` フィールドに定義。
...
```
.claude/skills/daily-digest-thumbnail/
├── SKILL.md          # このファイル
