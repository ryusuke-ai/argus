---
name: sns-qiita-writer
description: Qiita 記事を生成。sns-publisher から自動委譲。
---

# SNS Qiita Writer

Qiita 向けの技術記事を、リサーチから最適化まで4フェーズで生成する。
...
[Phase 2] 構成設計 → work/structure.json
    ↓
[Phase 3] コンテンツ生成 → work/qiita-article.json
    ↓
[Phase 4] 最適化 → 最終コンテンツ
```
...
| Phase 3 | @phases/phase3-content.md   | 直接実行（Claude） | qiita-article.json |
...
| ファイル                            | 用途           |
| ----------------------------------- | -------------- |
| @prompts/qiita-article-generator.md | コンテンツ生成 |
...
| @schemas/qiita-article.schema.json | Phase 3 |
