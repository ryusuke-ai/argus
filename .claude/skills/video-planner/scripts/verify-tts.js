...
const DEFAULT_WHISPER_MODEL = join(homedir(), '.whisper-models', 'ggml-base.bin');
...
async function mergeWavFiles(partsDir, outputPath) {
  ...
  const listContent = wavFiles.map(f => `file '${join(absPartsDir, f)}'`).join('\n');
  await writeFile(listPath, listContent);

  try {
    // ffmpegで結合
    execSync(`ffmpeg -y -f concat -safe 0 -i "${listPath}" -acodec libmp3lame -q:a 2 "${outputPath}"`, {
      stdio: 'pipe'
    });
...
async function transcribeAudio(audioPath, modelPath) {
  ...
  try {
    execSync(`ffmpeg -y -i "${audioPath}" -ar 16000 -ac 1 -acodec pcm_s16le "${wavPath}" 2>/dev/null`, {
      stdio: 'pipe'
    });
  } catch (error) {
...
function preflightUppercaseCheck(dialogue) {
  ...
  const mixedCasePattern = /\b[A-Z][a-z]+[A-Z][A-Za-z]*\b|\b[a-z]+[A-Z][A-Za-z]*\b|\b[A-Z]{2,}[a-z]+[A-Za-z]*\b/g;

  // ハイフン付き英単語を検出 (obsidian-skills, claude-code など)
  // 2文字以上-2文字以上のパターン
  const hyphenatedPattern = /\b[A-Za-z][A-Za-z0-9]*-[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z][A-Za-z0-9]*)?\b/g;

  // 一般的に認識される略語（例外リスト）
  const allowedAbbreviations = new Set([
    'AI', 'PC', 'IT', 'OK', 'ID', 'TV', 'CD', 'DVD', 'USB', 'URL',
    'CPU', 'GPU', 'RAM', 'ROM', 'SSD', 'HDD', 'LAN', 'WAN', 'WiFi',
    'HTML', 'CSS', 'JSON', 'XML', 'SQL', 'PHP', 'AWS', 'GCP',
    'API', 'SDK', 'IDE', 'CLI', 'GUI', 'MVP', 'MVC', 'ORM',
    'PR', 'QA', 'PM', 'HR', 'CEO', 'CTO', 'CFO', 'COO',
    'BGM', 'DM', 'NG', 'SNS', 'iOS', 'PDF', 'JPG', 'PNG', 'GIF',
    'MB', 'GB', 'TB', 'KB', 'Hz', 'MHz', 'GHz',
  ]);
  ...
  console.log('\n╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║ 📋 プレフライトチェック: 英単語の発音問題検出                        ║');
  console.log('╚══════════════════════════════════════════════════════════════════════╝');
  ...
  if (problemWords.length > 0) {
    ...
    for (const [word, info] of problemWords) {
      const segList = info.segments.slice(0, 3).map(i => i + 1).join(', ');
      const more = info.segments.length > 3 ? ` 他${info.segments.length - 3}件` : '';
      const typeLabel = info.type === 'uppercase' ? '大文字' : info.type === 'hyphenated' ? 'ハイフン' : '混合ケース';
      ...
      issues.push({
        type: info.type === 'uppercase' ? 'uppercase_word' : info.type === 'hyphenated' ? 'hyphenated_word' : 'mixed_case_word',
        word,
  ...
  console.log('  ╭────────────────────────────────────────────────────────────────────╮');
  console.log('  │ 💡 対処方法:                                                       │');
  console.log('  │    1. dialogue-fixed.json が自動生成されます                       │');
  console.log('  │    2. dialogue-fixed.json で TTS を再生成                          │');
  console.log('  │    3. テロップには dialogue.json（オリジナル）を使用               │');
  console.log('  │    4. --auto-dict オプションで未知単語を自動辞書登録               │');
  console.log('  ╰────────────────────────────────────────────────────────────────────╯\n');
...
async function autoRegisterDictionary(words) {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║ 📚 LLM API経由で英単語の読み方を自動取得・辞書登録                   ║');
  console.log('╚══════════════════════════════════════════════════════════════════════╝');
  console.log(`\n対象単語: ${words.join(', ')}`);
...
function detectMismatches(transcription, dialogue) {
  ...
  for (const { word, context, segmentIndex } of watchWords) {
    ...
    if (!normalizedTranscript.includes(normalizedWord)) {
      ...
      console.log(`  ✗ 「${word}」が認識されていない (segment ${segmentIndex})`);
    }
  }

  // 既知の読み間違いパターン
  const knownMisreadings = [
    { original: '指原莉乃', wrongPatterns: ['笹原里野', '笹原 里野', '笹原りの'], correct: 'サシハラリノ' },
    { original: '快進撃', wrongPatterns: ['会心劇', '会心 劇'], correct: 'カイシンゲキ' },
    { original: '清華大学', wrongPatterns: ['成果大学', '成果 大学', '精華大学'], correct: 'シンカダイガク' },
  ];
...
async function registerDictionary(entries, dryRun = false) {
  ...
  for (const word of dictionaryWords) {
    console.log(`  ${word.word} → ${word.yomi} (アクセント: ${word.accent}, モーラ: ${word.numMoras})`);
  }
...
async function regenerateTTS(dialogue, partsDir, affectedSegments) {
  ...
  for (const idx of affectedSegments) {
    ...
    console.log(`  ${String(idx + 1).padStart(3, '0')}: ${seg.text.substring(0, 30)}...`);
  }

  // batch-tts.js を実行（--indices オプションで特定セグメントのみ）
  try {
    const indicesArg = affectedSegments.map(i => i + 1).join(',');
    execSync(`node "${batchTtsPath}" --input "${tempPath}" --indices "${indicesArg}"`, {
      stdio: 'inherit',
...
async function main() {
  ...
  try {
    ...
    if (uppercaseIssues.length > 0) {
      ...
      await writeFile(fixedDialoguePath, JSON.stringify(fixedDialogue, null, 2));
      ...
      console.log('   2. merge-script.js には dialogue.json（オリジナル）を使用');

      // 検証結果に大文字英単語の問題を含める
      const preflightResultPath = join(outputDir, 'tts-preflight-result.json');
      await writeFile(preflightResultPath, JSON.stringify({
        timestamp: new Date().toISOString(),
        dialogue: options.dialogue,
        fixedDialogue: fixedDialoguePath,
        uppercaseIssues,
        replacementCount,
        message: 'dialogue-fixed.jsonが生成されました。TTS再生成にはこちらを使用してください。テロップにはオリジナルのdialogue.jsonを使用します。'
      }, null, 2));
    ...
    // 発音の違いを検出
    console.log('\n発音の違いを検出中...');
    const { mismatches, transcribedText, originalWords } = detectMismatches(transcription, dialogue);
    ...
    const verificationResult = {
      ...
      mismatches,
      suggestedDictionary,
      affectedSegments: [...new Set(mismatches.map(m => m.segmentIndex).filter(i => i >= 0))]
    };
    ...
    // TTS再生成
    if (options.regenerate && !options.dryRun && verificationResult.affectedSegments.length > 0) {
      await regenerateTTS(dialogue, options.partsDir, verificationResult.affectedSegments);
    }
