 ...
 * ダイアログ生成（Phase 2）・演出計画（Phase 3）に使用
 * Codexが失敗した場合はZ.ai (GLM)にフォールバック
 *
 * 使用方法:
 *   node glm-json.js --prompt <prompt-file> --schema <schema-file> [--context <context-file>...] [--output <output-file>]
 */
...
function buildCodexPrompt(outputPath) {
  let prompt = `あなたは動画台本・演出の専門家です。以下の指示に従ってJSONファイルを生成してください。

## タスク
指定されたJSON Schemaに従った形式で台本/演出計画を生成し、以下のファイルパスに保存してください:
${outputPath}
...
- 日本語で出力してください
- 自然な会話の流れを意識してください
- セリフは読み上げやすい長さにし、長い場合は複数セリフに分割してください
...
async function callZaiApi(outputPath) {
  ...
  const systemPrompt = `あなたは動画台本・演出の専門家です。指定されたJSON形式で出力してください。
...
async function main() {
  try {
    ...
    // スキーマ名をファイル名から推測（dialogue.schema.json → dialogue）
