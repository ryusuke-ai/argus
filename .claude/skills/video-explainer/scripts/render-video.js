  ...
  return new Promise((resolve, reject) => {
    ...
    const request = protocol.get(url, (response) => {
      ...
      if (
        ...
        downloadUrl(response.headers.location, destPath)
          .then(resolve)
          .catch(reject);
        return;
      }
...
async function loadScript(inputPath, fps) {
  ...
  const rawScenes = await Promise.all(
    script.scenes.map(async (scene, _index) => {
      ...
      if (scene.video) {
        ...
        if (existsSync(videoPath)) {
          try {
            ...
            console.warn(
              `Warning: Could not get video metadata for ${videoPath}:`,
              err.message,
            );
          }
        }
      ...
      const isSlideTransition =
        scene.transition === "slideLeft" || scene.transition === "slideRight";
      ...
      if (
        scene.highlight &&
        typeof scene.highlight === "object" &&
        scene.highlight.sound
      ) {
  ...
  const watermark =
    script.watermark !== false ? script.watermark || "logo" : null;

  // publicフォルダにアセットをコピー
  const { publicDir, assetMap } = await preparePublicDir(
    scriptDir,
    rawScenes,
    bgm,
    watermark,
...
async function main() {
  ...
  if (!options.input) {
    console.error(
      "Usage: node render-video.js --input <script.json> --output <output.mp4>",
    );
    process.exit(1);
  }
  ...
  const {
    ...
    fontSrc,
    watermarkSrc,
  } = await loadScript(options.input, options.fps);
  ...
  // 出力パス
  const outputPath =
    options.output || `./output/${title.replace(/\s+/g, "_")}.mp4`;
  ...
  console.log(
    `Total frames: ${totalFrames} (${(totalFrames / options.fps).toFixed(1)}s)`,
  );
