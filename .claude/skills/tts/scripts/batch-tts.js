...
try {
  charactersConfig = JSON.parse(
    await readFile(CHARACTERS_CONFIG_PATH, "utf-8"),
  );
} catch {
  // 設定ファイルがなくても動作可能
}
...
async function main() {
  ...
  try {
    ...
    // 出力ディレクトリの準備
    const outputDir =
      config.outputDir || join(__dirname, "../../../agent-output/tts-output");
    ...
    for (let i = 0; i < config.segments.length; i++) {
      ...
      if (!text) {
        console.warn(
          `警告: セグメント ${i + 1} にテキストがありません。スキップします。`,
        );
        continue;
      }
      ...
      console.log(
        `  テキスト: "${text.length > 40 ? text.slice(0, 40) + "..." : text}"`,
      );
      if (textChanged) {
        console.log(
          `  TTS用: "${cleanedText.length > 40 ? cleanedText.slice(0, 40) + "..." : cleanedText}"`,
        );
      }
