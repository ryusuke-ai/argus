...
function loadDictionary() {
  ...
  if (data.includes('<<<<<<< ') || data.includes('=======') || data.includes('>>>>>>> ')) {
    console.error('');
    console.error('╔══════════════════════════════════════════════════════════════════════╗');
    console.error('║ ⛔ ERROR: マージコンフリクトが検出されました                          ║');
    console.error('╚══════════════════════════════════════════════════════════════════════╝');
    console.error('');
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      throw new Error('辞書ファイルの形式が不正です（配列である必要があります）');
    }
    ...
    console.error('');
    console.error('╔══════════════════════════════════════════════════════════════════════╗');
    console.error('║ ⛔ ERROR: 辞書ファイルの解析に失敗しました                           ║');
    console.error('╚══════════════════════════════════════════════════════════════════════╝');
    console.error('');
    console.error(`ファイル: ${DICT_FILE}`);
...
async function cmdAutoAdd(words, options = {}) {
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      child.on('error', (err) => {
        reject(new Error(`Failed to spawn get-english-reading.js: ${err.message}`));
      });
    });
...
async function cmdHealthcheck() {
  ...
  try {
    ...
    console.log('   ⛔ COEIROINKに接続できません（起動しているか確認してください）');
    hasError = true;
  }
  ...
  if (duplicates.length > 0) {
    console.log(`   ✅ ${duplicates.length} 件の大文字小文字バリエーションあり（正常）`);
    duplicates.slice(0, 5).forEach(d => console.log(`      - "${d.existing}" と "${d.new}"`));
  ...
  console.log('\n' + '═'.repeat(70));
  if (hasError) {
    console.log('⛔ ヘルスチェックに失敗しました。上記のエラーを修正してください。');
    process.exit(1);
  } else {
...
async function cmdVerify(words) {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║ 🎤 発音確認（estimate_prosody API）                                   ║');
  console.log('╚══════════════════════════════════════════════════════════════════════╝\n');
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.error('  healthcheck           - 辞書の健全性チェック（TTS前に必須）');
      console.error('  add <word> <yomi>     - Add dictionary entry manually');
      console.error('  auto-add <words...>   - Auto-add with LLM API (get reading automatically)');
      console.error('  check <words...>      - Check if words are registered');
      console.error('  verify <words...>     - Verify pronunciation in COEIROINK');
