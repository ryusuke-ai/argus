---
name: sns-note-writer
description: note 記事を生成。sns-publisher から自動委譲。
---

# SNS Note Writer

note 向けの記事コンテンツを、リサーチから最適化まで4フェーズで生成する。
...
[Phase 2] 構成設計 → work/structure.json
    ↓
[Phase 3] コンテンツ生成 → work/note-article.json
    ↓
[Phase 4] 最適化 → 最終コンテンツ
```
...
| Phase 3 | @phases/phase3-content.md   | 直接実行（Claude） | note-article.json |
...
| ファイル                           | 用途           |
| ---------------------------------- | -------------- |
| @prompts/note-article-generator.md | コンテンツ生成 |
...
| @schemas/note-article.schema.json | Phase 3 |
