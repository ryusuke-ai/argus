...
| **Railway**   | Git push でデプロイ。VPS も対応 | レンタルオフィス（自由度高い） | フルスタックアプリ              | **採用**     |
| **EC2 (AWS)** | 最も自由度が高い仮想サーバー    | ビルの一室を自由に改装         | 大規模・細かい制御が必要な場合  | —            |
| **Heroku**    | PaaS の元祖。簡単だがやや割高   | 家具付きマンション             | プロトタイプ・小規模アプリ      | —            |

> **平たく言うと**: 「プログラムを動かすためのレンタルサーバー」のこと。Argus は Railway を選んだ。Git にコードを push するだけで自動的に新しいバージョンが公開される手軽さと、VPS として自由にプロセスを管理できる柔軟さが両立している。

#### Argus での採用理由

Railway VPS を採用。Slack Bot は常時接続（Socket Mode）で動き続ける必要があり、サーバーレスでは対応できない。Railway は GitHub 連携の自動デプロイを備えつつ VPS モードで PM2（プロセスマネージャー）を使った常駐サーバーが運用できる。月額コストも EC2 より予測しやすい。
...
> **平たく言うと**: Argus では Cloudflare Tunnel 経由でアクセスするため、Cloudflare のネットワークを通るが、AI 処理やデータベースアクセスは中央のサーバー（Railway VPS）で行っている。Edge 側ではセキュリティ（Cloudflare Access による認証）と通信の暗号化を担当している。
...
> **平たく言うと**: Argus では Cloudflare Tunnel がこの「マンションのエントランス」の役割を果たしている。外部からは Cloudflare を経由しないと Railway 上のサーバーにアクセスできない。さらに Cloudflare Access でメール認証をかけているので、許可された人だけがダッシュボードに入れる。
...
| **Railway**        | Git push で自動デプロイ | ワンタッチ出荷ボタン        | **採用**     |

> **平たく言うと**: Argus は Railway の GitHub 連携を使っている。main ブランチにコードを push すると、Railway が自動的にビルドしてデプロイしてくれる。複雑な CI 設定ファイルを書かなくても「push したら本番に反映」が実現できる。
...
> **平たく言うと**: Argus の現在の規模では Railway + Cloudflare の GUI 設定で十分に管理できているため、IaC ツールは導入していない。サービスが成長してインフラが複雑化した場合に導入を検討する領域。
...
> **平たく言うと**: Argus はルート `.env` ファイルに環境変数を一本化して管理している。Railway のデプロイ環境では Railway の環境変数管理機能を使い、`.env` ファイル自体はリポジトリにコミットしない（`.gitignore` で除外）。個人開発の現在の規模では十分だが、チーム開発に移行する際は Secrets Manager 系の導入を検討する。
...
| ------------------ | ---------------------------- | -------------------------------- |
| ホスティング       | Railway VPS                  | Git push デプロイ + VPS の自由度 |
...
| CI/CD              | Railway GitHub 連携          | シンプルな自動デプロイ           |
