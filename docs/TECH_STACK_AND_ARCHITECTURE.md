...
| **ACID トランザクション**（データ整合性保証） | ネイティブ対応                   | 4.0+ で対応だが制限あり              | 対応（InnoDB）                        | 制限付き（25 項目まで）           |
...
| **JSON サポート**                             | `jsonb` でインデックス可能       | ネイティブ                           | `JSON` 型あり                         | ネイティブ                        |
...
| **ESM 対応**                         | ネイティブ ESM                  | CJS 前提で後付け対応                      | 後付け           | 遅い・不安定      | ネイティブ     | -            |
| **型推論**                           | `select()` の戻り値が自動推論   | `prisma generate` で生成                  | 弱い             | デコレータベース  | 強い           | なし         |
| **SQL との距離**                     | SQL の写し（学習コスト低）      | 独自 DSL                                  | SQL に近い       | 独自 API          | SQL に近い     | SQL そのもの |
...
| **複雑なクエリ**（Window 関数, CTE） | 自然に書ける                    | `$queryRaw` に逃げがち                    | 書けるが型が弱い | 難しい            | 書ける         | 自由自在     |
| **エコシステム成熟度**               | 急成長中                        | 最大・最も成熟                            | 長い歴史         | 停滞気味          | 成長中         | -            |

### 決定: Drizzle ORM

ESM ネイティブ + TypeScript 型推論 + SQL に近い + マイグレーションツール（drizzle-kit）の組み合わせが、Argus の要件にぴったりだった。
...
| **MCP 対応**           | ネイティブ（`mcpServers` オプション）              | なし（自前で接続）            | コミュニティプラグイン     | なし                     | なし                                 |
...
| **モデルロックイン**   | Claude 専用                                        | Claude 専用                   | マルチモデル対応           | マルチモデル対応         | OpenAI 専用                          |
...
Argus の中核要件「AI が自律的にツールを使いながら作業を進める」に最も直接的に対応していた。特に MCP ネイティブサポートと Max Plan による追加コストなしの運用が決定的だった。
...
- SDK のバージョンアップで breaking change が起きる可能性（まだ v0.x）
...
| ------------------------ | ------------------------------------ | ------------------------------ | -------------------------- | ----------------------------- |
| **幽霊依存の防止**       | シンボリックリンクで厳密分離         | ホイスティングで幽霊依存が発生 | PnP モードで防止可能       | ホイスティング（npm 互換）    |
| **ディスク効率**         | Content-addressable store で重複排除 | 各プロジェクトにコピー         | PnP なら node_modules 不要 | 各プロジェクトにコピー        |
| **workspace プロトコル** | `workspace:*` で成熟                 | v7+ で対応                     | 対応                       | 対応                          |
| **インストール速度**     | 高速（ハードリンク）                 | 中程度                         | 高速（PnP）                | 最速                          |
...
> - Express 5 の async/await ネイティブ対応
> - 薄い API サーバーにおけるフレームワーク選択の考え方

### 背景（なぜ選ぶ必要があったか）

Orchestrator は 4 つのエンドポイント + Cron スケジューラという薄い API サーバー。フレームワークの高機能さよりも、シンプルさとエコシステムの広さが重要だった。

### 選択肢の比較

| 比較軸               | Express 5                    | Hono                       | Fastify                        | NestJS                         |
| -------------------- | ---------------------------- | -------------------------- | ------------------------------ | ------------------------------ |
| **async/await 対応** | ネイティブ（v5 で対応）      | ネイティブ                 | ネイティブ                     | ネイティブ                     |
| **パフォーマンス**   | 中程度                       | 高い（Edge 最適化）        | 高い（Express の 2-3 倍）      | 中程度（Express ベース）       |
| **エコシステム**     | 最大（ミドルウェアが豊富）   | 成長中                     | 中程度                         | 大きい（エンタープライズ向け） |
| **学習コスト**       | 最低（ほぼ全員が知っている） | 低い                       | 低い                           | 高い（DI, デコレータ）         |
| **型安全性**         | 基本的（@types/express）     | 優秀（型付きルーティング） | 良い（スキーマバリデーション） | TypeScript ネイティブ          |
...
- 4 つのエンドポイント + Cron スケジューラという薄い API に対して、Express 5 の async/await ネイティブ対応で十分
...
- Express 5 で async/await がネイティブ対応し、エラーハンドリングが簡潔に
- ミドルウェアのエコシステムが最も充実
...
Argus は完全 ESM プロジェクト。テストフレームワークも ESM にネイティブ対応している必要がある。1,200 以上のテストを高速に実行できることも重要な要件。

### 選択肢の比較

| 比較軸              | Vitest 4                                | Jest                                         | Mocha                   | node:test        |
| ------------------- | --------------------------------------- | -------------------------------------------- | ----------------------- | ---------------- |
| **ESM 対応**        | ネイティブ                              | `--experimental-vm-modules` が必要（不安定） | 対応だが設定が必要      | ネイティブ       |
...
| **Watch モード**    | 高速（HMR ベース）                      | 中程度                                       | 対応                    | `--watch`        |
...
| **Docker サポート**  | ネイティブ        | なし                   | Dockerfile 対応 | Dockerfile 対応   | 完全対応        |
...
| **S3 互換 API**    | 完全互換                         | ネイティブ           | 互換レイヤーあり           | 完全互換           |
...
- S3 と比べて一部の高度な機能（S3 Select, Glacier 等）が未対応
...
| **Vitest**        | ヴィテスト                  | JavaScript/TypeScript のテストフレームワーク。高速で ESM ネイティブ対応           |
